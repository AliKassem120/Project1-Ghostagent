...
function GhostMark({ className = 'w-7 h-7' }: { className?: string }) {
    const id = useId();
    const gradId = `ghost-brand-grad-${id}`;
    const glowId = `ghost-brand-glow-${id}`;
    ...
    return (
        ...
        >
            <defs>
                ...
                <linearGradient id={glowId} x1="20" y1="0" x2="20" y2="40" gradientUnits="userSpaceOnUse">
            ...
            />
            {/* Left eye */}
            <circle cx="14.5" cy="17" r="2.5" fill={`url(#${gradId})`} />
            <circle cx="13.8" cy="16.3" r="0.8" fill="white" opacity="0.7" />
            {/* Right eye */}
            <circle cx="25.5" cy="17" r="2.5" fill={`url(#${gradId})`} />
