...
Known customer: ${known ? `name=${known.name}, phone=${known.phone}` : 'new customer'}.`,
    ...
    // Parse date/time from the raw message even if the LLM fails to extract date_text.
    // This fixes cases like "Monday 11 am" repeating the time question forever.
    ...
    if (!order.productName) {
        if (extracted.product_name) {
            ...
            reply = products.length > 0
                ? orderReply({ key: 'ask_product', language, config })
                : orderReply({ key: 'unknown_product', language, config });
        }
