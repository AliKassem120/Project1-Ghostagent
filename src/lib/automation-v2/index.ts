...
export async function handleAutomationMessage(input: AutomationInput): Promise<AutomationResult> {
    ...
    try {
        ...
        v2log.webhookOutcome({
            ...
            appointmentInsertSuccess: result.debug.dbWriteSuccess && input.workspaceType === 'appointments' && result.actions.includes('appointment_created') ? true : undefined,
            appointmentUpdateSuccess: result.debug.dbWriteSuccess && input.workspaceType === 'appointments' && (result.actions.includes('appointment_rescheduled') || result.actions.includes('post_context_modify_success')) ? true : undefined,
            appointmentCancelSuccess: result.debug.dbWriteSuccess && result.actions.includes('appointment_cancelled') ? true : undefined,
            orderInsertSuccess: result.debug.dbWriteSuccess && input.workspaceType === 'ecommerce' && result.actions.includes('order_created') ? true : undefined,
            orderUpdateSuccess: result.debug.dbWriteSuccess && input.workspaceType === 'ecommerce' && result.actions.includes('post_context_modify_success') ? true : undefined,
            orderCancelSuccess: result.debug.dbWriteSuccess && result.actions.includes('order_cancelled') ? true : undefined,
