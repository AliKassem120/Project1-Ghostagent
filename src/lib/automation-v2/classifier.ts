...
const HANDOFF_PATTERNS = /\b(human|real\s*person|manager|speak\s*to\s*someone|talk\s*to\s*someone|operator|agent|representative|live\s*chat|bade\s*7ake\s*7ada|bade\s*hake\s*hada)\b/i;
...
const CANCEL_PATTERNS = /\b(cancel|refund|return|undo|remove\s*(my\s*)?(order|appointment|booking)|ilghe|ilghi|lghiya|badde\s*ilghi|bade\s*cancel)\b/i;

const PRODUCT_PATTERNS = /\b(price|how\s*much|cost|stock|available|in\s*stock|do\s*you\s*(have|sell|carry)|what\s*do\s*you\s*(have|sell|offer)|ade|adde|ade\s*7a2o|kam|bikam|bkam|fi\s*3andkon|3andkon|shu\s*3andkon|show\s*me|what\s*products|catalog|menu)\b/i;

const ORDER_PATTERNS = /\b(i\s*want|i('ll|\s*will)\s*(take|buy|get|order)|deal|add\s*to\s*cart|buy\s*(it|this|that)|order\s*(it|this|that)|bade|badde|bde|bedde|bdee|take\s*it|i('m|\s*am)\s*interested|yes\s*(i\s*want|please|deal))\b/i;
...
export function classifyIntent(
    ...
    if (GREETING_PATTERNS.test(msg) || msg.length <= 3) {
    ...
    if (msg.split(/\s+/).length <= 4 && !msg.includes('?')) {
        // Short message, probably a product/service name
        if (businessType === 'ecommerce') {
