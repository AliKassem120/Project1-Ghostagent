...
export function classifyIntent(
    ...
    if (isArabiziStyle(arabizi.detectedStyle)) {
        ...
        if (arabizi.hints.wantsAppointment || arabizi.hints.asksAvailability) return { intent: businessType === 'appointments' ? 'booking_intent' : 'general_chat', language: detected, confidence: 'regex' };
        if (arabizi.hints.asksService) return { intent: businessType === 'appointments' ? 'service_inquiry' : 'general_chat', language: detected, confidence: 'regex' };
        if (arabizi.hints.wantsToBuy || arabizi.hints.asksStock || arabizi.hints.asksPrice) return { intent: businessType === 'ecommerce' ? 'order_intent' : 'general_chat', language: detected, confidence: 'regex' };
    }

    if (handoffKeywords.length > 0 && handoffKeywords.some(kw => msgLower.includes(kw.toLowerCase()))) {
    ...
    if (msg.split(/\s+/).length <= 4 && !msg.includes('?')) {
        return businessType === 'ecommerce'
            ? { intent: 'product_inquiry', language, confidence: 'regex', subject: msg }
            : { intent: 'service_inquiry', language, confidence: 'regex', subject: msg };
    }
